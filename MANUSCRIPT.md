...
Path dependency is a well established idea in research (Arthur, 1988), but it is rarely tested at the small, street level scale used in this study.
...
**Figure 3.** Historical coal mining sites overlaid on present day 15 minute walking accessibility in Bochum. This map lays out the spatial relationship between old industrial sites and current accessibility, which the following sections test statistically.
...
**Figure 4.** Box and whisker plot comparing distance to old coal mining sites for high and low accessibility nodes. Contrary to the original hypothesis, low accessibility areas sit farther from the old mining sites than high accessibility areas do.
...
**Figure 5.** Local Moran's I cluster map. Blue nodes are significant "cold spot" (Low Low) clusters; gold nodes are significant spatial outliers (High Low); grey nodes are not significant. Triangles mark historical coal mines and diamonds mark worker colonies.
...
**Figure 6.** Side by side comparison of Bochum and Essen across the 3 statistical tests. The raw reversed effect and the spatial clustering result both replicate; the confound independence result does not. Essen's own Local Moran's I cluster map, directly comparable to Figure 5 for Bochum, is provided as Supplementary Figure S6.
...
The following supplementary figures accompany this article and are provided in a separate supplementary file, consistent with the journal's data and reproducibility policy:

- **Supplementary Figure S1.** Map of Essen's administrative boundary in North Rhine-Westphalia, Germany, directly comparable to Figure 1 (Bochum).
- **Supplementary Figure S2.** Essen's historical coal mining geography: 4 coal mines and 4 worker colonies, digitized from KuLaDig and German Wikipedia, directly comparable to Figure 2 (Bochum).
- **Supplementary Figure S3.** Effect size comparison of distance to nearest historical site by accessibility group (left) and mean distance to nearest historical site at 10, 15, and 20 minute thresholds (right).
- **Supplementary Figure S4.** Historical industrial sites overlaid on present day 15 minute walking accessibility in Essen, directly comparable to Figure 3 (Bochum).
- **Supplementary Figure S5.** Distance to historical sites for high and low accessibility nodes in Essen, comparable to Figure 4 (Bochum).
- **Supplementary Figure S6.** Local Moran's I cluster map for Essen, directly comparable to Figure 5 (Bochum).

## Data Availability

The data and code that support the findings of this study are openly available in a public repository. To preserve the integrity of the double-anonymized review process, the repository link is withheld during peer review and will be made available upon acceptance
